$(document).ready(function() {
    ...
    function isValidPlate(vnr) {
      ...
      // Plate type rules:
      // - General motor vehicles (types A/B/C): 2 Latin letters + 1..4 digits (1..9999)
      ...
      //   no more than 3 identical symbols in a row, and letters/numbers are in blocks (no A1B2).
      if (!vnr) {
        return false;
      }
      ...
      // Reject 4+ identical symbols in a row (CSDD: no more than three).
      if (/(.)\1\1\1/.test(vnr)) {
        return false;
      }
  ...
  function performCarInfoRequest(vnr) {
    ...
    //           '<div class="alert alert-warning">Atvainojamies, darba laiks saīsinājies, lūgums izvēlēties citu pieraksta laiku</div>' +
    //           '</div></div></div>';
    //
