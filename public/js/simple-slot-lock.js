...
class SimpleSlotLock {
    constructor() {
        ...
        this.inactivityTimeout = 120000; // 2 minutes of inactivity
    ...
    setupActivityTracking() {
        ...
        // Track activity only when a reservation is active
    ...
    resetInactivityTimer() {
        // If warning is already shown, do not reset until action is taken
    ...
    /**
     * Reservation timer (by inactivity)
     */
    startReservationTimer() {
        ...
        this.reservationTimer = setInterval(() => {
            ...
            const timeUntilWarning = this.inactivityTimeout - inactiveTime;
            
            // Show warning 60 seconds before inactivity timeout
            if (timeUntilWarning <= 60000 && timeUntilWarning > 0 && !this.warningShown) {
                this.warningShown = true;
                this.showTimeWarningModal(Math.floor(timeUntilWarning / 1000));
            }
            
            // Update timer in warning modal
            if (this.warningShown && timeUntilWarning > 0) {
                this.updateWarningTimer(Math.floor(timeUntilWarning / 1000));
            }
