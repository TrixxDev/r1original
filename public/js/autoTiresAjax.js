$(document).ready(function() {
  ...
  let season = (catalogSegment.indexOf('vasaras-riepas') === 0) ? 1 : 2;
  window['fastsearch'] = '';
  ...
  window['top_enabled'] = ($.urlParam('top') !== null) ? '&top=' + $.urlParam('top') : '&top=show';
  let pageNr = 1;
  ...
  let clickedTop = ($.urlParam('top') !== null) || window['top_enabled'].length > 0;
...
// SHOW VIEW DEPENDING ON LOCAL STORAGE VALUE
  ...
  function canUseServerRenderedList() {
    if (typeof pageLoaded !== 'undefined' && pageLoaded !== undefined) {
      return false;
    }
    if ($('#js-product-list .products .tire-table-row').length === 0) {
      return false;
    }
    if (window['selected_tires'] !== '' || window['show_selected'] !== '' || window['page'] !== '') {
      return false;
    }
    if (window['fastsearch'] !== '' || window['top_enabled'] !== '') {
      return false;
    }
    if (initialDualSizeEnabled || isDualSizeEnabled()) {
      return false;
    }
    if (localStorage.getItem('show_type') === 'grid') {
      return false;
  ...
  function drawHTML(data, skipHtmlReplace) {
  ...
  async function fetchWithTimeout() {
    ...
    if (pageLoaded === 0) {
      const queryFromUrl = location.search.slice(1);
      if (queryFromUrl) {
        url = '/api/tires/auto/' + season + '?' + brand + window['fastsearch'] + queryFromUrl + window['table_type'] + window['top_enabled'];
      } else {
        url = '/api/tires/auto/' + season + '?' + brand + 'd1=' + window['d1'] + '&d2=' + window['d2'] + '&d3=' + window['d3'] + dualSizeQuery() + window['fastsearch'] + window['availability'] + window['code'] + window['type'] + window['fuelEco'] + window['wetRoad'] + window['noise'] + window['selected_tires'] + window['show_selected'] + window['top_enabled'] + window['page'] + window['table_type'];
      }
    }
  ...
  $(document).on('click', '.cart-shopping-button', function() {
    if (!admin) {
      ...
      let ajaxUrl = '/' + resolveCatalogSegment();
