...
/** Cenas grozā — veseli eiro (kā katalogā), nevis 440.00 */
function formatCartPrice(value) {
    const n = parseFloat(String(value).replace(/,/g, '')) || 0;
...
/**
 * Удаляет строку товара из DOM корзины
 */
function removeCartItemFromDom(link) {
    const itemRow = link.closest('.cart-item-container');
    if (!itemRow) {
        return;
    }

    const nextEl = itemRow.nextElementSibling;
    itemRow.remove();

    if (nextEl && nextEl.classList.contains('separator')) {
        nextEl.remove();
    }
}

/**
 * Показывает пустую корзину без перезагрузки страницы
 */
function applyEmptyCartUI() {
    const container = document.querySelector('.card.cart-container');
    if (container) {
        container.querySelectorAll('.cart-item-container').forEach(function(el) {
        ...
        if (!container.querySelector('.cart-overview.js-cart')) {
            const emptyDiv = document.createElement('div');
            emptyDiv.className = 'cart-overview js-cart';
            emptyDiv.innerHTML = '<span class="no-items">Jūsu grozs ir tukšs</span>';

            const continueLink = container.querySelector('a.label');
            if (continueLink) {
                container.insertBefore(emptyDiv, continueLink);
            } else {
    ...
    const summaryColumn = document.querySelector('.cart-grid-right');
    if (summaryColumn) {
        summaryColumn.style.display = 'none';
    }

    const gridBody = document.querySelector('.cart-grid-body');
    if (gridBody) {
        gridBody.classList.remove('col-lg-8');
        gridBody.classList.add('col-lg-12');
    }
    ...
    $('.blockcart.cart-preview').removeClass('active').addClass('inactive');
}

/**
 * Обновляет корзину после удаления товара
 */
function updateCartAfterRemove(data, link) {
    removeCartItemFromDom(link);
    ...
    const montageSpan = document.querySelector('.cart-montage-choice label.cart-delivery-label span');
    if (montageSpan && data.total_items) {
        montageSpan.textContent = data.total_items + (data.total_items === 1 ? ' Riepai' : ' Riepām');
    }
}
    ...
    document.querySelectorAll('.remove-from-cart').forEach(link => {
        ...
        link.addEventListener('click', function(e) {
            ...
            confirmButton.addEventListener('click', function() {
                ...
                // POST + JSON: без redirect и без reload
                $.ajax({
                    url: originalHref,
                    method: 'POST',
                    headers: {
                    ...
                    success: function(data) {
                        ...
                        updateCartAfterRemove(data, link);
                    },
                    error: function(xhr) {
                        const message = xhr.responseJSON && xhr.responseJSON.error
                            ? xhr.responseJSON.error
                            : 'Произошла ошибка при удалении товара. Пожалуйста, попробуйте еще раз.';
...
window.updateCartUI = function(data, productId, quantity, price) {
    ...
    if (priceElement) {
        ...
        priceElement.innerHTML = `<strong>€ ${formatCartPrice(totalPrice)}</strong>`;
    }
    ...
    if (data.subtotal != null && data.subtotal !== '') {
        $('#cart-subtotal-products .value').html('€ ' + formatCartPrice(data.subtotal));
    }
    ...
    if (hasDelivery) {
        ...
        if (hasFitting) {
            ...
            if (data.total != null && data.total !== '') {
                $('.cart-total .value').html('€ ' + formatCartPrice(data.total));
            }
        }
    }
...
window.checkShipping = function(qty = null) {
    ...
    let totalPrice = parseFloat($('#cart-subtotal-products .value').html().trim().replace('€ ', '').replace(/,/g, '')) || 0;
        ...
        success: function(data) {
            ...
            if (city == 1) {
                if (totalPrice >= 115) {
                    ...
                    $('.cart-total .value').html('€ ' + formatCartPrice(totalPrice));
                    $('input[name="delivery_price"]').removeAttr('value');
                } else {
                    $('#cart-subtotal-shipping #shipping_price').html('€ ' + formatCartPrice(data.cartOptions.shipping_price));
                    $('.cart-total .value').html('€ ' + formatCartPrice(totalPrice + parseFloat(data.cartOptions.shipping_price)));
    ...
    window.cartState.ajaxRequests.checkFitting = $.ajax({
        ...
        success: function(response) {
            ...
            if (cartOptions.fitting_price > 0) {
                $('#cart-subtotal-montage').show();
                $('#cart-subtotal-montage #shipping_price').html('€ ' + formatCartPrice(cartOptions.fitting_price));
            } else {
                $('#cart-subtotal-montage').hide();
            }
