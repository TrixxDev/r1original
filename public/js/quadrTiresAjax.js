...
// SHOW VIEW DEPENDING ON LOCAL STORAGE VALUE
  ...
  function updateSelectedTiresState() {
    ...
    brand = (brand === 'Ražotājs' || brand === 'Visi' || !brand) ? '' : 'brand=' + brand + '&';
    window['newUrl'] = buildQuadrCatalogUrl();
  ...
  function buildQuadrCatalogUrl() {
    brand = $('select.tire-brand option:selected').val();
    brand = (brand === 'Ražotājs' || brand === 'Visi' || !brand) ? '' : 'brand=' + brand + '&';
    return '/' + pathParts[1] + '/search?' + brand + 'd1=' + window['d1'] + '&d2=' + window['d2'] + '&d3=' + window['d3'] + window['availability'] + window['selected_tires'] + window['show_selected'] + window['top_enabled'] + window['page'];
  }
