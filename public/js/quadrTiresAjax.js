...
// SHOW VIEW DEPENDING ON LOCAL STORAGE VALUE
  ...
  function loadItems()
  {
    ...
    if (fastsearchInput.length > 0) {
      $.get('/api/tires/quadrSplitInput/' + fastsearchInput, '', function(data) {
        window['d1'] = data.d1;
        window['d2'] = data.d2;
        window['d3'] = data.d3;
      });
    ...
    let url = '/api/tires/quadr/?' + brand + 'd1=' + window['d1'] + '&d2=' + window['d2'] + '&d3=' + window['d3'] + window['fastsearch'] + window['availability'] + window['table_type'] + window['selected_tires'] + window['show_selected'] + window['page'];

    if (pageLoaded === 0) {
      url = '/api/tires/quadr/?' + brand + window['fastsearch'] + location.search.slice(1) + window['table_type'];
    }
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $(document).find('th.tire-table-checkbox').children().on('click', function() {
          ...
          newUrl = '/' + pathParts[1] + '/search?' + brand + 'd1=' + window['d1'] + '&d2=' + window['d2'] + '&d3=' + window['d3'] + window['availability'] + window['selected_tires'] + window['show_selected'] + window['page'];
