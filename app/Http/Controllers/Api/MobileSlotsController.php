...
class MobileSlotsController extends Controller
{
    ...
    /**
     * Global slot search for mobile (takenby JSON + comment). Same row shape as GET /slots after enrich.
     */
    public function searchSlots(Request $request)
    {
        ...
        $needle = $validated['q'];
        $like = '%'.$this->escapeLikeWildcards($needle).'%';

        $q = DB::table('slots')->where(function ($sub) use ($like) {
            $sub->where('takenby', 'like', $like)
        ...
        $byDate = $slots->groupBy(function ($row) {
            return substr((string) ($row->date ?? ''), 0, 10);
        });

        $merged = collect();
        foreach ($byDate->sortKeysDesc() as $date => $group) {
            if ($date === '' || strlen($date) < 10) {
                continue;
            }
            $enriched = $this->enrichSlotsWithWallTime($group->values(), $date);
            $merged = $merged->concat($this->sortSlotsForDisplay($enriched));
        }

        return response()->json(['slots' => $merged->values()]);
    }

    /** Escape % and _ for SQL LIKE (MySQL default escape). */
