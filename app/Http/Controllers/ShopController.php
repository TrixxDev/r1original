...
class ShopController extends Controller
{
    ...
    public function shop_done($order_number, PurchaseTrackingService $purchaseTracking, GoogleAdsConversionService $googleAds) {
	    ...
	    $order = DB::table('orders_')->where('id', $order->id)->first();
            ...
            $this->dispatchGoogleAdsPurchase($googleAds, $order);
    ...
    public static function updateCartInDatabase($total_sum = 0, $cartData = null)
    {
        // Check if the user is logged in
        $userId = Auth::id();
        // Get the current session ID (instead of using a cookie)
	...
	// Prefer the freshly built cart payload; Cookie::get('cart') still holds
        // the previous request value because Cookie::queue() only applies on response.
        $cartData = $cartData ?? Cookie::get('cart');
    ...
    public function ajaxChangeQty(Request $request)
    {
        ...
        // Если товар существует в корзине, обновляем его количество
        if (isset($cart['products'][$product_id])) {
            $cart['products'][$product_id]['quantity'] = $request->qty;
            // Если передана цена, обновляем её
	...
	$cartJson = json_encode($cart);
        Cookie::queue('cart', $cartJson, 43200);

	Self::updateCartInDatabase($totalSum, $cartJson);
        ...
        // Формируем более полный ответ для функции updateCartUI
        $response = [
            'total_items' => array_sum(array_column($cart['products'], 'quantity')),
            'total_sum' => $totalSum,
            'subtotal' => $totalSum,
            'total' => $totalSum,
            'cart_item' => [
	...
	$removed = false;
	    ...
	    // Capture the name before removing the item from the cart.
	...
	if ($request->ajax() || $request->expectsJson() || $request->header('X-Requested-With') === 'XMLHttpRequest') {
            ...
            $qtyTotal = $isEmpty ? 0 : array_sum(array_column($cart['products'], 'quantity'));
