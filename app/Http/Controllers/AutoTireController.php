...
//        }
  ...
  public function api_tires(Request $request, $season) {
    ...
    try {
      ...
      $show_selected = $filterContext['show_selected'];
      $topTires = $filterContext['topTires'];
      ...
      if ($request->table_type === 'list') {
        if ($tires->count() > 0) {
          ...
          foreach ($tires as $index => $tire) {
            ...
            if ($fullSize !== $tire->fullSize) {
              ...
              $html .= '<h2 class="tire-brand-name">' . $tire->fullSize . '</h2>';
            }
            $isSelected = in_array((string) $tire->tire_id, $selectedTires, true);
            $html .= '<tr class="tire-table-row' . ($isSelected ? ' selected' : '') . '" role="row">';
            $html .= '<th scope="row" class="tire-table-checkbox"><input type="checkbox" value="' . $tire->tire_id . '" name="product_ids[]" class="tire-table-checkbox" aria-label="' . e('Atlasīt: ' . $tire->fullTitle) . '"' . ($isSelected ? ' checked' : '') . '></th>';
        ...
        foreach ($tires as $tire) {
          ...
          $brand = $tire->fullSize;
          if ($cbrand != $brand) {
            $html .= '</div><h2 class="tire-brand-name grid-t" style="margin-left: 5px;">' . $brand;
            if ($index == 0) {
  ...
  public function tires_ajax_dual_kit(Request $request): \Illuminate\Http\JsonResponse
  {
    ...
    $payload['kit'] = [
      ...
      'items' => [
        $this->buildDualKitCartItemPayload($tireA, $kitQuantity, 'Priekšējā ass'),
        $this->buildDualKitCartItemPayload($tireB, $kitQuantity, 'Aizmugurējā ass'),
      ],
    ];
  ...
  protected function renderCatalogView(Request $request)
  {
    ...
    return view('tires.auto.tires', array_merge(compact('code_array'), $catalogData));
  }
  ...
  protected function resolveCatalogPageData(Request $request): array
  {
    ...
    $countQuery = $this->buildApiTiresBaseQuery($season);
    $this->applyApiTireCatalogFilters($countQuery, $filterContext);
    $totalItems = (int) $countQuery->count('auto_tires.tire_id');
    $totalPages = (int) ceil($totalItems / $perPage);

    $listQuery = $this->buildApiTiresBaseQuery($season)
      ->selectRaw('auto_tires.*, auto_tires.quantity as tire_quantity, auto_treads.*, ' . $this->partnerStockColumnSql() . ' as stock_quantity, auto_brands.title as api_brand_title, auto_brands.slug as api_brand_slug');
    $this->applyApiTireCatalogFilters($listQuery, $filterContext);
    $tires = $listQuery->orderBy('d3', 'ASC')
      ->orderBy('d1', 'ASC')
      ->orderBy('d2', 'ASC')
      ->orderBy('price2', 'DESC')
      ->skip($offset)
  ...
  protected function normalizeFilterTokenList($value): array
  {
    if ($value === '' || $value === null) {
      return [];
    }
  ...
  protected function buildApiFilterContextFromRequest(Request $request): array
  {
    ...
    $d1b = $request->input('d1b', $this->d1b ?? '');
    $d1b = ($d1b == 'Visi' || $d1b === null) ? '' : $d1b;
    $d2b = $request->input('d2b', $this->d2b ?? '');
    $d2b = ($d2b == 'Visi' || $d2b === null) ? '' : $d2b;
    $d3b = $request->input('d3b', $this->d3b ?? '');
    $d3b = ($d3b == 'Visi' || $d3b === null) ? '' : $d3b;
    $dualSize = $request->boolean('dual')
      || ($d1b !== '' && $d1b !== null)
      || ($d2b !== '' && $d2b !== null)
      || ($d3b !== '' && $d3b !== null);
    ...
    if ($topTires === null && $this->isAutoTireCatalogRequest($request)) {
      $topTires = 'show';
    }
    ...
    $codeCombinations = [];
    $codeRequest = $request->code ?? $this->code ?? null;

    if (!is_null($codeRequest) && $codeRequest !== '' && $codeRequest !== []) {
      $rawCodes = is_array($codeRequest) ? $codeRequest : explode(' ', (string) $codeRequest);
      $this->code = $rawCodes;
      $categorizedCodes = [];
      ...
      foreach ($categorizedCodes as $value) {
        if (is_array($value)) {
          $hasArrays = true;
          $temp = [];
    ...
    $typeInput = $request->type ?? $this->type ?? '';
    $this->type = $selectedTypes = $this->normalizeFilterTokenList($typeInput);
    ...
    $fuelInput = $request->fuel ?? $this->fuel ?? '';
    $this->fuel = $selectedFuel = $this->normalizeFilterTokenList($fuelInput);

    $wetInput = $request->wet ?? $this->wet ?? '';
    $this->wet = $selectedWet = $this->normalizeFilterTokenList($wetInput);

    $noiseInput = $request->noise ?? $this->noise ?? '';
    $this->noise = $selectedNoise = $this->normalizeFilterTokenList($noiseInput);
  ...
  protected function renderCatalogListHtml(
    ...
    $seasonTitle = $season === 1 ? 'Vasaras riepas' : 'Ziemas riepas';
    $typeColumn = $season === 2
    ...
    $html .= '<span class="text-uppercase flipped-title tire-brand-name" style="color: black">' . e($seasonTitle) . '</span>';
    ...
    foreach ($tires as $index => $tire) {
      if ($fullSize !== $tire->fullSize) {
        ...
        $html .= '<table' . $tableId . ' class="table table-striped summer-sorter tires-table table-hover tablesorter">';
        $html .= $this->renderAutoTireTableColgroup($hasTypeColumn);
        $html .= '<thead class="tires-thead sticky-table"><tr>';
        $html .= '<th scope="col"></th>';
        $html .= '<th scope="col" class="table-tire-name-cell">Brends / modelis</th>';
        ...
        $html .= '<th scope="col" class="hidden-sm-down table-tire-param-cell"><div class="tire-table-icon icon-tire-rain" title="Slapjš segums"></div></th>';
        ...
        $html .= '<th id="store-price-button" scope="col" class="text-center catalog-price-th"><span class="hidden-sm-down">Veikala cena</span><span class="hidden-md-up">Veik.</span></th>';
        $html .= '<th id="store-sale-button" scope="col" class="text-center catalog-price-th"><span class="hidden-sm-down">Akcijas cena</span><span class="hidden-md-up">Akc.</span></th>';
        $html .= '<th scope="col" class="hidden-sm-down text-center">Piezīmes</th>';
        $html .= '<th scope="col"></th>';
        $html .= '<th scope="col"><div class="tire-table-icon icon-question" title="Pieejamība" data-toggle="tooltip"></div></th>';
        $html .= '</tr></thead>';
        $html .= '<tbody' . $tbodyId . '>';
        $html .= '<h2 class="tire-brand-name">' . e($tire->fullSize) . '</h2>';
        $tableOpen = true;
        $fullSize = $tire->fullSize;
      }

      $html .= $this->renderAutoTireListRow($tire, $selectedTires, $typeColumn);
    }
  ...
  protected function buildDualSizeApiResponseHtml(
    ...
    if ($request->table_type === 'list') {
      ...
      $html .= '<div class="tire-image-container">';
      if ($pairs !== []) {
        $html .= '</div><h2 class="tire-brand-name grid-t" style="margin-left: 5px;">';
        $html .= '<span style="margin: 0 auto;"></span>';
        $html .= '<button type="button" class="btn-sm btn-outline-danger hidden-md-up sm-filter-btn" data-toggle="modal" data-target="#mobileFilterModal">Filtrs</button></h2>';
  ...
  {
    if (!$dualPairMode) {
      return '<div class="container"><div class="col-md-12 mt-1 alert alert-danger">Ar šādiem parametriem nav atrasta neviena pozīcija.</div></div>';
    }
    ...
    return '<div class="container dual-size-empty-results">'
      ...
      . '<strong>Nav atrasts neviens komplekts ar abiem izmēriem.</strong>'
      . '<p class="mb-0 mt-2">Meklējam vienādu protektoru, kuram pieejams gan <strong>' . $sizeA . '</strong>, gan <strong>' . $sizeB . '</strong> (pa 2 gab. katram izmēram).</p>'
      . '<ul class="dual-size-empty-tips mb-0 mt-2">'
      . '<li>Pārbaudiet aizmugurējās ass diametru — bieži jāizvēlas <em>17</em>, nevis <em>17C</em>.</li>'
      . '<li>Noņemiet papildu filtrus (ražotājs, pieejamība, kods).</li>'
      . '<li>Noņemiet atzīmi «Otrs izmērs», lai meklētu tikai pēc viena izmēra.</li>'
      . '</ul>'
      . '</div></div>';
  }
  ...
  protected function renderDualSizePairKitTotalFoot(Autotire $sizeA, Autotire $sizeB, bool $hasTypeColumn): string
  {
    ...
    return '<tfoot class="dual-size-pair-tfoot">'
      ...
      . '<span class="dual-size-pair-total-label">Kopā: <span class="dual-size-pair-total-amount">' . e($total) . ' €</span></span>'
  ...
  protected function renderAutoTireListRow(
    ...
    if ($tire->season == 2) {
      ...
      if ($tire->type == 2) {
        $html .= '<span class="tippy lisi-tooltip type-explain" data-type="2" data-tippy-content="<div style=\'padding: 5px;\'><span style=\'color: black; font-size: 15px;\'>' . \App\Helper\Tires::codeExplain('radžojamu tips') . '</span></div>"><img src="/images/radzeb.png" alt="ms"></span>';
      }
      if ($tire->type == 3) {
        $html .= '<span class="tippy lisi-tooltip type-explain" data-type="3" data-tippy-content="<div style=\'padding: 5px;\'><span style=\'color: black; font-size: 15px;\'>' . \App\Helper\Tires::codeExplain('ar radzēm tips') . '</span></div>"><img src="/images/radzea.png" alt="ms"></span>';
      }
      if ($tire->type == 4) {
  ...
  protected function renderDualSizePairGridBlock(array $pair, array $selectedTires, int $season): string
  {
    ...
    $html .= '<span class="dual-size-pair-total-label">Kopā: <span class="dual-size-pair-total-amount">' . e($kitTotal) . ' €</span></span>';
