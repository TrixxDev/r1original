...
//            }
  ...
  public function studs_image(Request $request, $id)
  {
    if ($request->hasFile('tread_image')) {
      ...
      $fileNameLarge   = $id . '-o';
//            dd($image);
      Image::make($image->getRealPath())->save('public/storage/studs/tread/' . $fileName . '.jpg');
      Image::make($image->getRealPath())
        ->resize(100, 100, function($constraint) {
          $constraint->aspectRatio();
        })->save('public/storage/studs/tread/' . $fileNameSmall . '.jpg');
      Image::make($image->getRealPath())
        ->resize(200, 200, function($constraint) {
          $constraint->aspectRatio();
        })->save('public/storage/studs/tread/' . $fileNameMed . '.jpg');
      Image::make($image->getRealPath())
        ->resize(1500, 1500, function($constraint) {
          $constraint->aspectRatio();
        })->save('public/storage/studs/tread/' . $fileNameLarge . '.jpg');
    }
