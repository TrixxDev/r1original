...
class ShopController extends Controller
{
  ...
  public function order($id)
  {
    ...
    foreach ($orderDetails->products as $article => $product) {
      $productModel = $this->getProductModel($product->url, $product->id);
      $tire = (object)[
        'tire_id' => $product->id,
        'article' => $article,
        'url' => $product->url,
        ...
        'price' => $product->price,
        'dotAvailable' => $productModel ? $productModel->dotAvailable : 'red',
        'stockAvailability' => $productModel ? $productModel->stockAvailability : 'Nav pieejams'
      ];
      $tires[] = $tire;
    }
  ...
  public function order_update(Request $request, $id)
  {
    ...
    // Atjaunojam automobiļa informāciju
    if ($request->car_brand || $request->car_model || $request->car_release_year || $request->car_engine_size) {
        $car_details = [
            'car_brand' => $request->car_brand,
            'car_model' => $request->car_model,
            'car_release_year' => $request->car_release_year,
            'car_engine_size' => $request->car_engine_size
        ];
        $order->car_details = json_encode($car_details);
    }
  ...
  private function getProductModel($url, $id)
  {
    if (!$url) {
      return (object)['dotAvailable' => '', 'stockAvailability' => ''];
    }

    // Определяем тип товара по URL
    if (strpos($url, '/vasaras-riepa/') !== false || strpos($url, '/ziemas-riepa/') !== false) {
      // Автомобильные шины
      $model = \App\Models\Autotire::find($id);
    } elseif (strpos($url, '/moto') !== false) {
      // Мотоциклетные шины
      $model = \App\Models\Moto::find($id);
    } elseif (strpos($url, '/kvadru') !== false) {
      // Квадроциклетные шины
      $model = \App\Models\Quadr::find($id);
    } elseif (strpos($url, '/lietie-diski') !== false) {
      // Диски - у них нет методов dotAvailable и stockAvailability
      return (object)['dotAvailable' => '', 'stockAvailability' => ''];
    } elseif (strpos($url, '/lielas') !== false) {
      // Большие шины
      $model = \App\Models\Bigtire::find($id);
    } else {
      // По умолчанию пробуем автомобильные шины
      $model = \App\Models\Autotire::find($id);
    }

    // Если модель не найдена, возвращаем пустые значения
    if (!$model) {
      return (object)['dotAvailable' => '', 'stockAvailability' => ''];
    }
