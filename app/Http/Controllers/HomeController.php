...
//      $ftp = uploadFTP("212.3.218.22", "r1_web", "RA5bgdGc", dirname(__DIR__, 3) . '/xml/pasutijums' . $xml_order . '.xml', "pasutijums$xml_order.xml");
      $uploadResult = uploadFTP($file, "pasutijums$xml_order.xml");
      ...
      if (!($uploadResult['ok'] ?? false)) {
        ...
        if ($uploadReason === 'missing_local_file') {
          return json_encode(['danger' => 'Pasūtījuma XML fails nav atrasts pirms FTP sūtīšanas.']);
        }
        return json_encode(['danger' => 'Pasūtījums nav nosūtīts uz Accrual (FTP upload, reason: ' . $uploadReason . ').']);
      }
