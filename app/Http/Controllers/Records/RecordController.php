<?php
  ...
  class RecordController extends Controller
  {
    ...
    public function __construct()
    {
      ...
      $this->endSendWpp = \Carbon\Carbon::create(date('Y'), date('m'), date('d'), 19, 00);
    ...
    public function reservations(Request $request) {
      ...
      $workingDays = NewWorkingDay::whereIn('date', $daysToShow)
				    ->orderBy('office_id')
				    ->orderBy('queue_id')
				    ->get();

      if (isset($request->date)) {
        $workingDays = NewWorkingDay::where('date', $date)
				      ->orderBy('office_id')
				      ->orderBy('queue_id')
				      ->get();
        $daysToShow[] = $date;
