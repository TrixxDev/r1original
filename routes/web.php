...
// Grozs
  ...
  Route::post('/shop/ajaxQtyUp', [App\Http\Controllers\ShopController::class, 'ajaxQtyUp'])->name('shop.ajaxQtyUp');
  Route::post('/shop/ajaxQtyDown', [App\Http\Controllers\ShopController::class, 'ajaxQtyDown'])->name('shop.ajaxQtyDown');
  ...
  Route::post('/cart/checkShipping', [App\Http\Controllers\ShopController::class, 'checkShipping']);
  Route::post('/cart/checkFitting', [App\Http\Controllers\ShopController::class, 'checkFitting']);
  Route::get('/cart/empty', function() {
    \Cart::destroy();

    return redirect()->back();
  })->name('cart.empty');
  Route::get('/cart/ajaxRefresh', [App\Http\Controllers\ShopController::class, 'ajaxRefresh'])->name('cart.ajaxRefresh');
  Route::post('/cart/ajaxChangeQty', [App\Http\Controllers\ShopController::class, 'ajaxChangeQty'])->name('cart.ajaxChangeQty');
  Route::post('/cart/ajaxQtyUp', [App\Http\Controllers\ShopController::class, 'ajaxQtyUp'])->name('cart.ajaxQtyUp');
  Route::post('/cart/ajaxQtyDown', [App\Http\Controllers\ShopController::class, 'ajaxQtyDown'])->name('cart.ajaxQtyDown');
...
// Одноразовый перенос file-сессий в MySQL (без Artisan). Альтернатива: /migrate-sessions.php?token=...
Route::get('/_migrate-sessions/{token}', function (string $token) {
  ...
  if (! request()->boolean('dry') && ($result['ok'] ?? false)) {
